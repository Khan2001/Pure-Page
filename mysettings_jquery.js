...
function loadLocalStorage_settings(){
	...
	//搜索引擎
	switch (searchEngine) {
		case "baidu":$('input[name="searchEngine"]:eq(0)').attr("checked","checked");break;
		case "sogou":$('input[name="searchEngine"]:eq(1)').attr("checked","checked");break;
		case "bing":$('input[name="searchEngine"]:eq(2)').attr("checked","checked");break;
		case "google":$('input[name="searchEngine"]:eq(3)').attr("checked","checked");break;
		case "360":$('input[name="searchEngine"]:eq(4)').attr("checked","checked");break;
	}
...
function saveBtn(){
	$('#saveBtn').click(function(){
		...
		setCookie(titleShow,titleColor,titleFont,searchEngine,backgroundUrl,hitokotoShow,hitokotoColor,hitokotoFont,allWebName,allWebUrl,allWebIcon,nameLength);
	});
	
}
function setCookie(titleShow,titleColor,titleFont,searchEngine,backgroundUrl,hitokotoShow,hitokotoColor,hitokotoFont,allWebName,allWebUrl,allWebIcon,nameLength){
	var second = new Date().getTime() + (365*24+8)*60*60*1000;
	var expires = new Date(second).toGMTString();
	localStorage.setItem("titleShow",titleShow);
	localStorage.setItem("titleColor",titleColor);
	localStorage.setItem("titleFont",titleFont);
	localStorage.setItem("searchEngine",searchEngine);
	...
	localStorage.setItem("allWebIcon",allWebIcon);
	localStorage.setItem("nameLength",nameLength);
	console.log(titleShow,titleColor,titleFont,searchEngine,backgroundUrl,hitokotoShow,hitokotoColor,hitokotoFont,allWebName,allWebUrl,allWebIcon,nameLength);
	alert('设置保存成功！');
}
